...
func newUrlBtn(content string, url string, typename larkcard.MessageCardButtonType) *larkcard.
	MessageCardEmbedButton {
	btn := larkcard.NewMessageCardEmbedButton().
		Type(typename).
		Url(url).
...
func SendErrorWriteCard(ctx context.Context, msgId *string,
	err error) {
	card, _ := newSendCard(
		withHeader("🧐 数据录入失败", larkcard.TemplateRed),
		withMainMd("检查您的输入是否正确"))

	replyCard(ctx, msgId, card)
}

func SendSuccessWriteCard(ctx context.Context, msgId *string,
	info string, link string) {
	card, _ := newSendCard(
		withHeader("👏 数据录入成功", larkcard.TemplateGreen),
		withMainMd("已经成功录入数据："),
		withMainMd(info),
		//withSplitLine(),
		withOneBtn(newUrlBtn("查看记录", link, larkcard.MessageCardButtonTypePrimary)))
	replyCard(ctx, msgId, card)
}
